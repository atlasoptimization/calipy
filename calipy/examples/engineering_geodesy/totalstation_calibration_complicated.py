...
coeff_meteo_p = 0.3
coeff_meteo_t = -1
delta_d_m = 1e-3*d_true*(coeff_meteo_t * Delta_T + coeff_meteo_p * Delta_P)
...
i_object = UnknownParameter(i_ns, name = 'i', init_tensor = torch.tensor([0.01]))
...
coeffs_t_object = UnknownParameter(coeffs_t_ns, name = 'coeffs_t',
                                   init_tensor = torch.tensor([0.005, 5]))
...
coeffs_m_object = UnknownParameter(coeffs_m_ns, name = 'coeffs_m',
                                   init_tensor = torch.tensor([-1.0, 0.3]))
...
phi_object = UnknownParameter(phi_ns, name = 'phi', init_tensor = torch.mean(data[:,:,:,1], dim = [1,2]))
...
class DemoProbModel(CalipyProbModel):
    ...
    def model(self, input_vars, observations = None):
        ...
        delta_d_m = 1e-3 * approx_d *(coeffs_m[:,:,:,0:1] * Delta_T 
