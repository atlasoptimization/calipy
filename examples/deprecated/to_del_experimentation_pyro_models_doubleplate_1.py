...
class NoiseModel():
    def __init__(self, noise_shape_dict):
    ...
    def forward(self, mean, observations = None):
        ...
        with pyro.plate('batch_plate_1', size = self.batch_shape[0], dim = -2):
            with pyro.plate('batch_plate_2', size = self.batch_shape[1], dim = -1):
                obs = pyro.sample('obs', self.noise_dist, obs = observations)
        return obs
    
class CombinedModel():
    def __init__(self, offset_shape_dict, noise_shape_dict):
        self.offset_model = OffsetModel(offset_shape_dict)
        self.noise_model = NoiseModel(noise_shape_dict)
...
combined_model_object = CombinedModel(offset_shape_dict, noise_shape_dict)
